...
(function () {
  ...
  function injectNavItem() {
    try {
      ...
      // Toggle submenu on click (Cortex binds handlers at init, not via delegation)
      ...
      onDispose(() => document.removeEventListener('click', closeHandler));
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // MODULE: DATE RANGE EXTRACTOR (stub)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const dateRangeExtractor = {
    ...
    init() {
      if (this._overlayEl) return;
      const overlay = document.createElement('div');
      overlay.id = 'ct-dre-overlay';
      overlay.className = 'ct-overlay';
      overlay.setAttribute('role', 'dialog');
      overlay.setAttribute('aria-modal', 'true');
      overlay.setAttribute('aria-label', 'Date Range Extractor');
      overlay.innerHTML = `
        <div class="ct-dialog">
          <h3>📅 Date Range Extractor</h3>
          <p style="color:var(--ct-muted);">Dieses Modul wird in einem zukünftigen Release implementiert.</p>
          <div style="margin-top:16px;text-align:right;">
            <button class="ct-btn ct-btn--close" id="ct-dre-close">✕ Schließen</button>
          </div>
        </div>
      `;
      document.body.appendChild(overlay);
      ...
      document.getElementById('ct-dre-close').addEventListener('click', () => this.hide());
      onDispose(() => this.dispose());
      log('Date Range Extractor stub initialized');
    },
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // MODULE: DAILY DELIVERY PERFORMANCE (stub)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const deliveryPerformance = {
    ...
    init() {
      ...
      overlay.setAttribute('role', 'dialog');
      overlay.setAttribute('aria-modal', 'true');
      overlay.setAttribute('aria-label', 'Daily Delivery Performance');
      overlay.innerHTML = `
        <div class="ct-dp-panel">
          <h2>📦 Daily Delivery Performance</h2>
          <p style="color:var(--ct-muted);">Dieses Modul wird in einem zukünftigen Release implementiert.</p>
          <div style="margin-top:16px;text-align:right;">
            <button class="ct-btn ct-btn--close" id="ct-dp-close">✕ Schließen</button>
          </div>
        </div>
      `;
      document.body.appendChild(overlay);
      ...
      onDispose(() => this.dispose());
      log('Delivery Performance stub initialized');
    },
    ...
    init() {
      ...
      overlay.innerHTML = `
        <div class="ct-dvic-panel">
          ...
          <p style="color:var(--ct-muted);">Dieses Modul wird in einem zukünftigen Release implementiert.</p>
          <div style="margin-top:16px;text-align:right;">
            <button class="ct-btn ct-btn--close" id="ct-dvic-close">✕ Schließen</button>
          </div>
        </div>
      `;
      document.body.appendChild(overlay);
      ...
      onDispose(() => this.dispose());
      log('DVIC Check stub initialized');
    },
  ...
  // ═══════════════════════════════════════════════════════════════════════════
  // MODULE: WORKING HOURS DASHBOARD (stub)
  // ═══════════════════════════════════════════════════════════════════════════
  ...
  const workingHoursDashboard = {
    ...
    init() {
      ...
      onDispose(() => this.dispose());
      log('Working Hours Dashboard stub initialized');
    },
